...
import me.nikitaklimkin.rest.account.dto.AddNewBrokerAccountRequest
import me.nikitaklimkin.rest.account.dto.AddNewCardAccountRequest
import me.nikitaklimkin.rest.account.dto.AddNewDepositAccountRequest
import me.nikitaklimkin.rest.account.dto.AddNewPiggyAccountRequest
import me.nikitaklimkin.rest.account.dto.UpdateBrokerAccountRequest
import me.nikitaklimkin.rest.account.dto.UpdateCardAccountRequest
import me.nikitaklimkin.rest.account.dto.UpdateDepositAccountRequest
import me.nikitaklimkin.rest.account.dto.UpdatePiggyAccountRequest
...
fun Route.accountRoute() {
    ...
    route(API_V1_ACCOUNTS) {
        ...
        post("/broker") {
            ...
            val body = call.receive<AddNewBrokerAccountRequest>()
        ...
        post("/card") {
            ...
            val body = call.receive<AddNewCardAccountRequest>()
            log.trace { "Receive request $body" }
            executeAddAccountRequest(
                either {
                    val userId = UserId.from(body.userId).bind()
        ...
        post("/deposit") {
            ...
            val body = call.receive<AddNewDepositAccountRequest>()
            log.trace { "Receive request $body" }
            executeAddAccountRequest(
                either {
                    val userId = UserId.from(body.userId).bind()
    ...
    post("/piggy") {
        ...
        val body = call.receive<AddNewPiggyAccountRequest>()
        log.trace { "Receive request $body" }
        executeAddAccountRequest(
            either {
                val userId = UserId.from(body.userId).bind()
                ...
                getAccounts.execute(GetAccountRequest(it))
                    .onLeft { error ->
                        when (error) {
                            ...
                            GetAccountError.AccountsNotFound -> call.respond(HttpStatusCode.NotFound)
                        }
                    }
                    .onRight { data ->
    ...
    put("/broker") {
        ...
        val requestBody = call.receive<UpdateBrokerAccountRequest>()
    ...
    put("/card") {
        ...
        val requestBody = call.receive<UpdateCardAccountRequest>()
        log.trace { "Receive request $requestBody" }
        executeUpdateAccountRequest(
            either {
                val accountId = AccountId.from(requestBody.accountId).bind()
    ...
    put("/deposit") {
        ...
        val requestBody = call.receive<UpdateDepositAccountRequest>()
        log.trace { "Receive request $requestBody" }
        executeUpdateAccountRequest(
            either {
                val accountId = AccountId.from(requestBody.accountId).bind()
    ...
    put("/piggy") {
        ...
        val requestBody = call.receive<UpdatePiggyAccountRequest>()
        log.trace { "Receive request $requestBody" }
        executeUpdateAccountRequest(
            either {
                val accountId = AccountId.from(requestBody.accountId).bind()
